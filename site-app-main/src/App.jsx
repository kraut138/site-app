...
import { queueOfflineRequest, getQueuedRequests, removeQueuedRequest, markQueuedRequestFailed } from "./offlineQueue.js";
...
export default function App() {
  ...
  // 데이터/와이파이 연결 상태 추적. 지하주차장·밀폐 공간 등 통신이 끊긴 곳에서
  // 하도급사가 제출한 "공사 확인 요청"은 서버로 못 보내는 대신 기기 내부 저장소(IndexedDB)에
  // 임시 저장되고, 여기서 연결이 복구되는 순간을 감지해 자동으로 서버 전송(동기화)한다.
  ...
  // 내부 저장소에 쌓여있던 요청들을 서버로 순서대로 재전송한다. 하나라도 실패하면(여전히 통신이
  // 불안정한 경우 등) 그 건은 저장소에 남겨두고 다음 기회에 다시 시도한다.
  const syncOfflineQueue = useCallback(async () => {
    ...
    try {
      ...
      for (const item of queued) {
        try {
          const created = await api.createInspections(item.data);
          setInspections((prev) => [...prev, ...created]);
          await removeQueuedRequest(item.localId);
        } catch (err) {
          await markQueuedRequestFailed(item.localId, err?.message || String(err));
        }
      }
    } finally {
  ...
  useEffect(() => {
    ...
    function handleOnline() {
      setIsOnline(true);
      notify("인터넷 연결이 복구되었습니다. 임시저장된 요청을 서버로 전송합니다…");
      syncOfflineQueue();
    }
    function handleOffline() {
      setIsOnline(false);
      notify("인터넷 연결이 끊겼습니다. 이후 제출하는 요청은 이 기기에 임시저장됩니다.");
    }
    window.addEventListener("online", handleOnline);
    window.addEventListener("offline", handleOffline);
    // 앱을 처음 열었을 때도(예: 지상으로 올라와 새로고침한 경우) 이미 온라인이면 바로 한 번 동기화한다.
    if (navigator.onLine) syncOfflineQueue();
    return () => {
      window.removeEventListener("online", handleOnline);
      window.removeEventListener("offline", handleOffline);
    };
  }, [notify, syncOfflineQueue, refreshPendingOfflineCount]);
  ...
  async function handleCreateInspection(data) {
    // 오프라인이면 바로 기기 내부 저장소에 임시저장하고, 화면에는 "오프라인 저장" 표시로 알린다.
    if (typeof navigator !== "undefined" && !navigator.onLine) {
      await queueOfflineRequest(data);
      await refreshPendingOfflineCount();
      return { offline: true, count: Array.isArray(data.units) ? data.units.length : 1 };
    }
    try {
      ...
      // 온라인으로 표시되지만 실제 전송이 실패하는 경우(터널 구간, 신호 약함 등)도
      // 데이터를 잃지 않도록 동일하게 내부 저장소에 담아둔다.
      await queueOfflineRequest(data);
      await refreshPendingOfflineCount();
      return { offline: true, count: Array.isArray(data.units) ? data.units.length : 1 };
    }

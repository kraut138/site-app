...
export default function Layout({ role, trueRole, viewAsRole, onSetViewAsRole, userProfile, onLogout, view, setView, badges = {}, isOnline = true, pendingOfflineCount = 0, syncing = false, children }) {
  const { t } = useLanguage();
      ...
      <aside className={`sidebar${roleClassName ? ` ${roleClassName}` : ""}`}>
        ...
        <div className="sidebar-foot">
          {(!isOnline || pendingOfflineCount > 0) && (
            <div className={"offline-status" + (isOnline ? " is-syncing" : " is-offline")}>
              ...
              <span className="offline-status-text">
                {!isOnline
                  ...
                  : syncing
                  ? t("offline.syncing")
                  : t("offline.pendingCount", { count: pendingOfflineCount })}
              </span>
            </div>
          )}
          {canSwitchRole && (
